...
function Newsletter() {
  return (
    <form
      action="/thank-you"
      className="rounded-2xl border border-amber-400 p-6 dark:border-amber-400/40"
    >
...
function EventComp({ role }: { role: EventData }) {
  let startLabel =
...
function Resume() {
  let resume: Array<EventData> = [
    {
      name: 'Flaktveit Open #2 2024',
      location: 'Flakveit',
      logo: logoPlanetaria,
      start: {
  ...
  return (
    <div className="rounded-2xl border border-amber-400 p-6 dark:border-amber-400/40">
      <h2 className="flex text-sm font-semibold text-zinc-900 dark:text-zinc-100">
        <BriefcaseIcon className="h-6 w-6 flex-none" />
        <span className="ml-3">Arrangementer vi er interessert i</span>
      </h2>
      <ol className="mt-6 space-y-4">
        {resume.map((role, roleIndex) => (
          <EventComp key={roleIndex} role={role} />
        ))}
      </ol>
